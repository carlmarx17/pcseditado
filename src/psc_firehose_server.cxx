...
//
// Physics (same as psc_firehose_light):
//   B along Z (HZ), variations in Y-Z plane.
//   Trigger: Ti_perp/Ti_par = 0.1 << 1 - 2/beta_i_par = 0.8  (strongly driven)
//
// Server parameters (vs light):
//   mass_ratio = 100   (me/mi = 1/100)
//   ppc        = 1000  (2x more statistics)
//   grid       = 256x256
//   domain     = 32 d_i x 32 d_i
//   nmax       = 100000
...
// ----------------------------------------------------------------------
// 4. Physical parameter setup — FIREHOSE instability
// ----------------------------------------------------------------------
void setupParameters()
{
  ...
  //   beta_i_par = 10  =>  threshold = 0.8
  //   Ti_perp/Ti_par = 0.1  =>  strongly driven (8x below threshold)
  // ----------------------------------------------------------------------
  g.BB                   = 1.0;
  g.Zi                   = 1.0;
  g.mass_ratio           = 100.0;
...
Grid_t* setupGrid()
{
  ...
  g.lambda_De = std::sqrt(g.Te_par / g.n); // Te isotropic for firehose
  ...
  // Ensure grid is divisible by 32 (4*8)
  grid_yz = ((grid_yz + 31) / 32) * 32;
  int ppc = envOrDefault("PSC_NICELL", kPPC);

  mpi_printf(MPI_COMM_WORLD,
             "[firehose_server] d_e=%.3g  d_i=%.3g  lambda_De=%.3g\n",
             g.d_e, g.d_i, g.lambda_De);
  mpi_printf(MPI_COMM_WORLD,
             "[firehose_server] domain=%.2f d_i  grid=%dx%d  ppc=%d\n",
             kDomainSizeDi, grid_yz, grid_yz, ppc);
  mpi_printf(MPI_COMM_WORLD,
             "[firehose_server] FIREHOSE: Ti_perp/Ti_par=%.2f  beta_i_par=%.1f"
             "  threshold=%.3f\n",
             g.Ti_perp_over_Ti_par, g.beta_i_par,
             1.0 - 2.0 / g.beta_i_par);
  mpi_printf(MPI_COMM_WORLD,
             "[firehose_server] Patches: 4x8=32  (run with mpirun -np 32)\n");
...
// ----------------------------------------------------------------------
// 6. Particle initialization — bi-Maxwellian (Ti_par >> Ti_perp)
// ----------------------------------------------------------------------
                         ...
                         Balance& bal, Grid_t*& gptr, Mparticles& mprts)
{
  partitionAndSetupParticles(
    ...
    [&](int kind, Double3, int, Int3, psc_particle_np& np) {
      ...
      if (kind == MY_ION) {
        npt.n    = g.n;
        npt.T[0] = g.Ti_perp; // perp to B
        npt.T[1] = g.Ti_perp;
        npt.T[2] = g.Ti_par;  // par to B (dominant)
      } else {
        npt.n    = g.n;
        npt.T[0] = g.Te_perp;
        npt.T[1] = g.Te_perp;
...
{
  mpi_printf(MPI_COMM_WORLD,
             "*** Firehose Instability — SERVER (32 cores, 256x256, 1000 ppc) ***\n");
  setupParameters();
  ...
  opp.data_dir   = ".";
  opp.basename   = "prt_firehose_server";
