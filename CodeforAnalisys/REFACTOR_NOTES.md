...
- The `Makefile` now runs the analysis scripts against `../build/src` data products and includes particle diagnostics.
- `psc_units.py` was added as a shared reference for unit conversions and derived plasma scales.
...
- Shared field source, different analyses:
  `fluctuationofmagneticfiel.py`, `mirror_physics.py`, and `spectral_analysis.py` all consume `pfd.*.h5`, but they are not interchangeable.
...
- Excluded:
  generated plots, `__pycache__`, editor settings, and the untracked root-level legacy `plotmirro.py`.
